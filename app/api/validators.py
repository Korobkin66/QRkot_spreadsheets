...
def check_charity_project_invested_sum(project:
                                       CharityProject, new_amount: int):
    if project.invested_amount > new_amount:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST,
            detail='Сумму не может быть ниже уже вложенной'
        )
...
def check_charity_project_already_invested(charity_project: CharityProject):
    if charity_project.invested_amount > 0:
        raise HTTPException(
            status_code=HTTPStatus.BAD_REQUEST,
            detail='Проект с не нулевым балансом не может быть удален'
        )
